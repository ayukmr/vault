...
if there exists a real number $d \ne 0$ such that $\sum^\infty_{n = 0} c_n d^n$ converges, then the series $\sum^\infty_{n = 0}{c_n x^n}$ converges absolutely for all $x$ such that $|x| < |d|$

since $\sum^\infty_{n = 0} c_n d^n$ converges, the $n$th term $c_nd^n \rightarrow 0$ as $n \rightarrow \infty$
...
the series $\sum^\infty_{n = N} |c_n x^n| \le \sum^\infty_{n = N} |\frac{x}{d}|^n$ converges if $|\frac{x}{d}| \le 1$, which it is
so $\sum^\infty_{n = N} |c_n x^n|$ must converge based on being squeezed by $\le$
...
$\frac{x^3}{2 - x} = \sum^\infty_{k = 3} \frac{1}{2^{k - 2}} x^k = 4\sum^\infty_{k = 3} (\frac{x}{2})^k$
